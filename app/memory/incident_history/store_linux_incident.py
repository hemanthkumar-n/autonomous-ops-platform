...
def store_linux_memory_incident(
    investigation: LinuxMemoryInvestigation,
) -> str:
    """
    Persist Linux memory investigation memory with semantic fallback.
    """

    target = (
        f"pid:{investigation.pid}"
        if investigation.pid is not None
        else "host"
    )
...
def store_linux_service_incident(
    investigation: LinuxServiceInvestigation,
) -> str:
    """
    Persist Linux service investigation memory with semantic fallback.
    """

    memory = _build_memory_investigation(
        investigation,
        domain="linux.service",
        target=investigation.service,
    )
    storage_dir = Path(settings.INCIDENT_HISTORY_DIR)
    storage_dir.mkdir(parents=True, exist_ok=True)

    timestamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S_%f")
    path = storage_dir / f"linux_service_incident_{timestamp}.json"
    path.write_text(
        json.dumps(memory.model_dump(mode="json"), indent=2),
        encoding="utf-8",
    )
...
def store_linux_boot_kernel_incident(
    investigation: LinuxBootKernelInvestigation,
) -> str:
    """
    Persist Linux boot/kernel investigation memory with semantic fallback.
    """
    ...
    timestamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S_%f")
    path = storage_dir / f"linux_boot_kernel_incident_{timestamp}.json"
    path.write_text(
        json.dumps(memory.model_dump(mode="json"), indent=2),
        encoding="utf-8",
    )
...
def store_linux_host_incident(
    investigation: LinuxHostInvestigation,
) -> str:
    """
    Persist correlated Linux host investigation memory.
    """
    ...
    timestamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S_%f")
    path = storage_dir / f"linux_host_incident_{timestamp}.json"
    path.write_text(
        json.dumps(memory.model_dump(mode="json"), indent=2),
        encoding="utf-8",
    )
